...
### Decision: npm publish with provenance
Publish via `actions/setup-node` (with `registry-url`) + `npm publish --provenance --access public`, authenticating with `NODE_AUTH_TOKEN=${{ secrets.NPM_TOKEN }}` and `permissions: id-token: write`. Provenance is free given the public repo and links the published artifact to the building workflow/commit. `prepublishOnly` (lint+build) still runs as a local safety net. **Alternative:** plain token publish without provenance — works, but provenance is low-cost supply-chain hardening.
